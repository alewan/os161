...
/*
 * Common code for cmd\_prog and cmd_shell.
 *
...
common_prog(int nargs, char **args)
{
	...
	//kprintf("waiting for pid");
	result = sys_waitpid(current->pid, &a, 6969, &b);
	if(result) return 0;
	//kprintf("asdfasdf");
...
cmd_df (int nargs, char ** args) {
	...
	//Check which bit should be on/off
	int mask = 0x1 << (flag - 1);

	//Check 2nd arg for on/off
	if(!(strcmp(args[2], "off"))) {
		dbflags &= ~mask;
	}
	else if(!(strcmp(args[2], "on"))) {
		dbflags |= mask;
	}
	else {
		kprintf("Usage: df nr on/off\n");
