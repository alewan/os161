...
#define INITIAL_CHILDSIZE 5
#define SECOND_CHILDSIZE 20
#define FINAL_CHILDSIZE 50
...
//Make a typedef available for threads
...
struct process {
	...
	//This is my PID
	int pid;
	//This is to determine whether or not the PID is used
	//0 means the PID is unused
	//1 means the process has exited but things are waiting on it
	//2 means the process is running
	int pidUsed;
	//This is the exitcode
	int exit_code;
	//This is the semaphore to see if I can exit or not
	struct lock* exitlock;
	//children
	//struct array* children;
	//the thread itself
	struct thread* thread;
	//if i have been waited on
	int waited;
};
...
//Added function to wake only one thread sleeping on a specific address to wake up
